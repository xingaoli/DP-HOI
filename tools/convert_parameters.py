...
def get_args():
    ...
    parser.add_argument(
        '--finetune_model', type=str, required=True, choices=('cdn', 'gen', 'hoiclip')
    )
    parser.add_argument(
...
def main(args):
    ...
    for k in list(ps['model'].keys()):
        print(k)
        if args.finetune_model == "gen" or args.finetune_model == "hoiclip": 
            if len(k.split('.')) > 1 and k.split('.')[1] == 'decoder':
                ps['model'][k.replace('decoder', 'instance_decoder')] = ps['model'][k].clone()
                del ps['model'][k]
            elif 'action_class_fc' in k:
                ps['model'][k.replace('action_class_fc', 'hoi_class_fc')] = ps['model'][k].clone()

    if args.finetune_model == "cdn":
    ...
    elif args.finetune_model == "gen" or args.finetune_model == "hoiclip":
